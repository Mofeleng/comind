...
export const Transcript = ({ sessionId }:Props) => {
    const trcp = useTRPC();
    const { data } = useQuery(trcp.sessions.getTranscript.queryOptions({
        id: sessionId
    }));
                ...
                <div className="flex flex-col gap-y-4">
                    { filteredData.map((item) => {
                        return (
                            <div
                                key={item.start_ts}
                                className="flex flex-col gap-y-3 hover:bg-muted p-4 rounded-md border"    
                            >
